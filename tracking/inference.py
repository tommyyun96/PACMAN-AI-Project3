...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        allPossible = util.Counter()
        for p in self.legalPositions:
            if p is self.getJailPosition(): continue
            for next_p, prob in self.getPositionDistribution(self.setGhostPosition(gameState, p)).items():
                allPossible[next_p] += prob * self.beliefs[p]
