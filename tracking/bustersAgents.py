...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = [beliefs for i,beliefs
                                            ...
                                            if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        # 1. Configure most likely positions of each ghost
        ghostPositions = [None]*sum(livingGhosts)
        for i in range(len(ghostPositions)):
            maxProb = 0
            chosenLoc = None
            for p, prob in livingGhostPositionDistributions[i].items():
                if chosenLoc is None or maxProb < prob:
                    chosenLoc = p
                    maxProb = prob
            ghostPositions[i] = chosenLoc

        nearestGhost = None
        minDist = 0
        for i in range(len(ghostPositions)):
            dist = self.distancer.getDistance(pacmanPosition, ghostPositions[i])
            if nearestGhost is None or minDist > dist:
                nearestGhost = i
                minDist = dist

        targetLoc = ghostPositions[nearestGhost]
        chosenAction = None
        minDistToGhost = 0
        for a in legal:
            dist = self.distancer.getDistance(targetLoc, Actions.getSuccessor(pacmanPosition, a))
            if chosenAction is None or minDistToGhost > dist:
                chosenAction = a
                minDistToGhost = dist
